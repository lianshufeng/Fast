...
import com.fast.dev.ucenter.core.dao.mongo.UserTokenLogDao;
...
public abstract class BaseUserService {
    ...
    @Autowired
    protected UserPushMessageHelper userPushMessageHelper;
    ...
    @Autowired
    private UserTokenLogDao userTokenLogDao;
    ...
    public boolean logout(String token, boolean all) {
        BaseToken baseToken = this.userTokenDao.query(token);
        if (baseToken == null) {
            return false;
        }
        //待删除列表
     ...
     * @return
     */
    public boolean checkBaseUserDisable(BaseUser baseUser) {
        UserDisableModel userDisableModel = baseUser.getUserDisable();
        if (userDisableModel == null) {
            return false;
        }
        if (this.dbHelper.getTime() > userDisableModel.getDisableTime() + userDisableModel.getCreateTime()) {
            this.UserManager.enable(new String[]{baseUser.getId()});
            return false;
        }
        return true;
    }
    ...
    public UserTokenModel createUserToken(TokenEnvironment env, BaseUser baseUser, long expireTime) {
        ...
        userToken.setEnvironment(env);
        UserTokenModel userTokenModel = null;
        if (this.userTokenDao.createUserToken(userToken, expireTime)) {


            //记录用户令牌的日志
            UserTokenLog userTokenLog = new UserTokenLog();
            BeanUtils.copyProperties(userToken, userTokenLog);
            this.userTokenLogDao.save(userTokenLog);
    ...
    private void setServiceToken(TokenEnvironment tokenEnvironment, ServiceTokenType serviceTokenType, String loginName, ServiceToken serviceToken, String code) {
        ...
        serviceToken.setEnvironment(tokenEnvironment);
    ...
    protected TokenState validateToken(ServiceToken serviceToken, String validateCode, boolean isValidateServiceToken) {
        ...
        //通过应用名取出对应的的配置
        ValidateDataConf validateDataConf = this.validateDataHelper.get(serviceToken.getEnvironment().getApp());
    ...
    public BasicServiceToken strongToken(String token, String code) {
        ...
        //创建时的环境而并非现在接收到的环境
        TokenEnvironment tokenEnvironment = serviceToken.getEnvironment();
        //业务令牌类型
    ...
    public long cleanUserToken(String uid, String[] ignoreUToken) {
        if (ignoreUToken == null) {
            ignoreUToken = new String[0];
        }
        ...
        for (BaseToken baseToken : baseTokens) {
            if (!ArrayUtils.contains(ignoreUToken, baseToken.getToken())) {
