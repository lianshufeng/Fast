...
public class UserManagerImpl extends BaseUserService implements UserManagerService {
     ...
     * @return
     */
    protected static BaseUserModel copyToBaseUserModel(BaseUser baseUser) {
        if (baseUser == null) {
            return null;
        }
    ...
    public UserTokenModel login(UserLoginType loginType, String loginName, String passWord, Long expireTime, TokenEnvironment env) {
        ...
        //用户被禁用
        if (super.checkBaseUserDisable(baseUser)) {
            return new UserTokenModel(TokenState.UserDisable);
        }
    ...
    public UserTokenModel createToken(String uid, Long expireTime, TokenEnvironment env) {
        ...
        return this.userManager.createUserToken(env, baseUser, expireTime);
    }
    ...
    public BaseUserModel updateLoginName(String uid, UserLoginType loginType, String loginName) {
        ...
        //如果更新登录名成功则发送广播
        if (baseUser != null && StringUtils.hasText(baseUser.getId())) {
            this.userPushMessageHelper.push(UserMessage.builder().uid(baseUser.getId()).type(UserMessageType.UpdateLoginName).build());
        }
        return copyToBaseUserModel(baseUser);
    }
    ...
    public boolean disable(String[] uid, long time, String reason) {
        UserDisableModel userDisableModel = new UserDisableModel();
        userDisableModel.setCreateTime(this.dbHelper.getTime());
        userDisableModel.setDisableTime(time);
        userDisableModel.setReason(reason);
        //更新用户数据
        boolean flag = this.baseUserDao.updateUserDisable(userDisableModel, uid) > 0;

        //清空缓存
        for (String u : uid) {
            this.userTokenDao.removeFromUid(u);
        }
        ...
        if (flag) {
            if (uid != null) {
                for (String u : uid) {
                    this.userPushMessageHelper.pushUserDisable(u);
                }
            }
        }
        return flag;
    ...
    public boolean enable(String[] uid) {
        boolean flag = this.baseUserDao.updateUserDisable(null, uid) > 0;
        if (flag) {
            if (uid != null) {
                for (String u : uid) {
                    this.userPushMessageHelper.pushUserEnable(u);
                }
            }
        }
        return flag;
