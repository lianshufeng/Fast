...
import com.fast.dev.ucenter.core.dao.mongo.ServiceTokenLogDao;
 ...
 */
@Service
public class UserServiceImpl extends BaseUserService implements UserService, LocalUserService {
    ...
    @Autowired
    private ServiceTokenLogDao serviceTokenLogDao;
...
//    @Transactional
    public long cleanTimeOutRecord(Class<? extends SuperEntity> entityCls, long time) {
        ...
        query.addCriteria(Criteria.where("createTime").lt(beforeTime));
        query.limit(10000);
        return this.mongoTemplate.remove(query, entityCls).getDeletedCount();
    }
}
